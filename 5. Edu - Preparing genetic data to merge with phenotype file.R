...
replicationdata2 <- lapply(replicationdata2[,1:40], as.numeric) 
...
famfinal <- fam[c("id", "IID", "FID", "PID", "MID", "sex", "kids_fathered", "split", "age_fbirth_multi", "num_kids", "childless", "z_NEB", "NEB", "age_centre", "edu_years", "college",  "qualification")]
...
finaldata <- merge(famfinalEX, replicationgenetic, by.famfinalEX=c("FID", "IID"), fam.finalEX=T)
write.csv(finaldata, file="EduRep_finaldata.csv", row.names=F, quote=F)
